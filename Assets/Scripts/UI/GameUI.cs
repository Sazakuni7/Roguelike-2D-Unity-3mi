...
public class GameUI : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text spawnersRestantesTmp;
    ...
    private void Update()
    {
        if (enVictoria && Input.anyKeyDown)
        {
            enVictoria = false;
            mensajeVictoriaTmp.gameObject.SetActive(false);

            // Disparamos evento para pasar al siguiente nivel
            GameEvents.TriggerNextLevel();
        }
    }
   ...
   public void ConectarJugador(Jugador jugador)
    ...
    public void MostrarVictoria()
    {
        if (mensajeVictoriaTmp != null)
        {
            ...
            mensajeVictoriaTmp.text = "¡Ganaste! Pulsa cualquier tecla para jugar otra vez";
            enVictoria = true;
        }

        // Pausamos el juego mientras el mensaje esté activo
        Time.timeScale = 0f;
    }
    ...
    public void ActualizarSpawnerTilesUI(int restantes, int total)
    {
        if (spawnersRestantesTmp != null)
        {
            spawnersRestantesTmp.text = $"Spawners restantes: {restantes}/{total}";
            Debug.Log($"[GameUI] UI actualizada: {spawnersRestantesTmp.text}");
        }
        else
        {
            Debug.LogWarning("[GameUI] spawnersRestantesTmp es NULL, no se puede actualizar la UI");
        }
    }
